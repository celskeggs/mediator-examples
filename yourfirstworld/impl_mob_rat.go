...
func NewMobRat(realm *types.Realm, params ...types.Value) *types.Datum {
	i := &MobRatImpl{}
	d := realm.NewDatum(i)
	...
	NewMobRatData(d, &i.MobRatData, params...)
	return d
}

func (t *MobRatImpl) Type() types.TypePath {
	return "/mob/rat"
}

func (t *MobRatImpl) Var(src *types.Datum, name string) (types.Value, bool) {
	switch name {
	case "type":
		return types.TypePath("/mob/rat"), true
	case "parent_type":
...
func (t *MobRatImpl) SetVar(src *types.Datum, name string, value types.Value) types.SetResult {
	switch name {
	case "type":
...
func (t *MobRatImpl) Chunk(ref string) interface{} {
	switch ref {
